...
CREDIT = {"dev": "ARYAN", "TG": "@AS_OWNER99"}
...
def xpopu_encode_varint(num: int) -> bytes:
    if num < 0:
...
def xpopu_create_field(num, val):
    if isinstance(val, bool):
        return xpopu_encode_varint((num << 3) | 0) + xpopu_encode_varint(int(val))
    if isinstance(val, int):
        return xpopu_encode_varint((num << 3) | 0) + xpopu_encode_varint(val)
    if isinstance(val, (str, bytes)):
        v = val.encode() if isinstance(val, str) else val
        return xpopu_encode_varint((num << 3) | 2) + xpopu_encode_varint(len(v)) + v
    if isinstance(val, dict):
        nested = xpopu_create_packet(val)
        return xpopu_encode_varint((num << 3) | 2) + xpopu_encode_varint(len(nested)) + nested
    return b""


def xpopu_create_packet(fields: dict) -> bytes:
    return b"".join(xpopu_create_field(k, v) for k, v in fields.items())


def xpopu_encrypt_api(plain_hex: str) -> str:
    plain  = bytes.fromhex(plain_hex)
    cipher = AES.new(AES_KEY, AES.MODE_CBC, AES_IV)
    return cipher.encrypt(pad(plain, AES.block_size)).hex()


def xpopu_aes_decrypt(data: bytes) -> bytes:
    try:
        cipher = AES.new(AES_KEY, AES.MODE_CBC, AES_IV)
...
def xpopu_decode_response(resp: bytes):
...
def xpopu_decode_jwt(token: str) -> dict:
    try:
...
def xpopu_build_major_login_body(access_token: str, open_id: str,
    ...
    return xpopu_create_packet(fields)


def xpopu_get_access_token(uid, password):
    headers = {
...
def xpopu_major_login_request(access_token, open_id,
                        region=DEFAULT_REGION, lang=DEFAULT_LANG):
    try:
        ob        = xpopu_get_version()
        plaintext = xpopu_build_major_login_body(access_token, open_id, region, lang)
        body      = bytes.fromhex(xpopu_encrypt_api(plaintext.hex()))

        ctx  = ssl._create_unverified_context()
        conn = http.client.HTTPSConnection(MajorLoginHost, context=ctx, timeout=12)
...
def xpopu_generate_jwt(uid, password):
    ...
    access_token, open_id = xpopu_get_access_token(uid, password)
    if not access_token or not open_id:
        return {"success": False, "message": "Invalid UID or Password", **CREDIT}

    raw, err = xpopu_major_login_request(access_token, open_id)
    if not raw:
        return {"success": False,
                "message": f"Login failed — {err or 'account may be banned'}",
                **CREDIT}

    res, mode, _ = xpopu_decode_response(raw)
    ...
    jwt_payload = xpopu_decode_jwt(res.token)
...
def xpopu(uid, password):
    return xpopu_generate_jwt(uid, password)


def xpopu_token(uid, password):
    result = xpopu_generate_jwt(uid, password)
...
def xpopu_api_token():
    ...
    result = xpopu_token(uid, password)
...
@app.get("/XPOPU")
def xpopu_kawsar_route():
    uid      = request.args.get("uid", "").strip()
    password = request.args.get("password", "").strip()
    return jsonify(xpopu(uid, password))


@app.post("/XPOPU")
def xpopu_kawsar_post():
...
@app.get("/update+")
def xpopu_version_up():
    v = xpopu_get_version()
    try:
        num  = int(v.replace("OB", ""))
        newv = f"OB{num + 1}"
        xpopu_set_version(newv)
        return jsonify({"success": True, "old": v, "new": newv, **CREDIT})
    except Exception:
...
@app.get("/version")
def xpopu_check():
    return jsonify({"ReleaseVersion": xpopu_get_version(), **CREDIT})
...
def xpopu_index():
    return jsonify({
        "name":    "XPOPU JWT API",
        "version": xpopu_get_version(),
        "routes":  {
            "/token":     "GET ?uid=&password=",
            "/XPOPU": "GET/POST ?uid=&password=",
...
if __name__ == "__main__":
    ...
    print(f"✅ XPOPU JWT API running on port {port}")
    print(f"📌 Version: {xpopu_get_version()}")
